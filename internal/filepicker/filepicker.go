...
func (m Model) readDir(path string, showHidden bool) tea.Cmd {
	return func() tea.Msg {
		...
		modTimes := make(map[string]time.Time, len(dirEntries))
		...
		sort.Slice(dirEntries, func(i, j int) bool {
			...
			// both files: mtime desc, name asc on tie (missing info => zero time, sinks to bottom)
			ti, tj := modTimes[di.Name()], modTimes[dj.Name()]
			if ti.Equal(tj) {
				return di.Name() < dj.Name()
			}
			return ti.After(tj)
